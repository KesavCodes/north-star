...
import { X, Check, CheckSquare, Clock, Hash, ShieldCheck, Heart, ChevronRight, CalendarDays, Repeat } from 'lucide-react-native';
...
export const AddTaskScreen = ({ navigation }: any) => {
  ...
  const colors = ['#2ECC71', '#3498DB', '#9B59B6', '#E67E22', '#E74C3C', '#95A5A6'];
  ...
  const handleSave = () => {
    ...
    addTask({
      ...
      category,
      isRoutine,
      date: isRoutine ? undefined : new Date().toISOString().split('T')[0],
      target,
      color: selectedColor,
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F8F9FA]" style={{ paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight : 0 }}>
      ...
      <ScrollView className="flex-1 px-5 mt-6" showsVerticalScrollIndicator={false}>
        ...
        {/* Frequency */}
        <Text className="text-sm font-semibold text-slate-800 mb-3">Frequency</Text>
        <View className="flex-row space-x-4 mb-6">
          ...
          >
            ...
            <Text className={`ml-2 font-medium ${isRoutine ? 'text-indigo-500' : 'text-slate-500'}`}>Routine</Text>
          </TouchableOpacity>
          <TouchableOpacity 
          ...
          >
            <CalendarDays color={!isRoutine ? '#14B8A6' : '#94A3B8'} size={18} />
            <Text className={`ml-2 font-medium ${!isRoutine ? 'text-teal-500' : 'text-slate-500'}`}>One-time</Text>
          </TouchableOpacity>
        </View>

        {/* Task Type */}
        <Text className="text-sm font-semibold text-slate-800 mb-3">Task Type</Text>
        <View className="flex-row justify-between space-x-3 mb-6">
          ...
          >
            <CheckSquare color={taskType === 'checkbox' ? '#3B82F6' : '#94A3B8'} size={28} />
            <Text className={`text-xs mt-2 font-medium ${taskType === 'checkbox' ? 'text-blue-500' : 'text-slate-400'}`}>Checkbox</Text>
          </TouchableOpacity>
          <TouchableOpacity 
          ...
          >
            <Clock color={taskType === 'timer' ? '#A855F7' : '#94A3B8'} size={28} />
            <Text className={`text-xs mt-2 font-medium ${taskType === 'timer' ? 'text-purple-500' : 'text-slate-400'}`}>Timer</Text>
          </TouchableOpacity>
          <TouchableOpacity 
          ...
          >
            <Hash color={taskType === 'counter' ? '#F97316' : '#94A3B8'} size={28} />
            <Text className={`text-xs mt-2 font-medium ${taskType === 'counter' ? 'text-orange-500' : 'text-slate-400'}`}>Counter</Text>
          </TouchableOpacity>
        </View>

        {/* Task Name */}
        <Text className="text-sm font-semibold text-slate-800 mb-3">Task Name</Text>
        <TextInput 
          value={name}
        ...
        {/* Category */}
        <Text className="text-sm font-semibold text-slate-800 mb-3">Category</Text>
        <View className="flex-row space-x-4 mb-6">
          <TouchableOpacity 
          ...
          >
            <ShieldCheck color={category === 'discipline' ? '#2ECC71' : '#94A3B8'} size={18} />
            <Text className={`ml-2 font-medium ${category === 'discipline' ? 'text-[#2ECC71]' : 'text-slate-500'}`}>Discipline</Text>
          </TouchableOpacity>
          <TouchableOpacity 
          ...
          >
            <Heart color={category === 'kindness' ? '#F39C12' : '#94A3B8'} size={18} />
            <Text className={`ml-2 font-medium ${category === 'kindness' ? 'text-[#F39C12]' : 'text-slate-500'}`}>Kindness</Text>
          </TouchableOpacity>
        </View>
        ...
        {taskType !== 'checkbox' && (
          <>
            <Text className="text-sm font-semibold text-slate-800 mb-3">Daily Target</Text>
            {taskType === 'timer' ? (
        ...
        {/* Reminder (Optional) */}
        <Text className="text-sm font-semibold text-slate-800 mb-3">Reminder (Optional)</Text>
