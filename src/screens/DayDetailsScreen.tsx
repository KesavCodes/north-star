      ...
      >
        ...
        <View className="flex-row justify-between mb-8 bg-white p-4 rounded-2xl shadow-sm border border-slate-50">
          <View className="items-center">
            ...
            <Text className="text-sm font-semibold text-slate-800 mt-1">
              {habits.filter((t) => logs[`${t.id}-${date}`]?.completed).length}/
              {habits.length}
            </Text>
          </View>
          <View className="items-center">
            ...
            <Text className="text-sm font-semibold text-slate-800 mt-1">
              {timers.filter((t) => logs[`${t.id}-${date}`]?.completed).length}/
              {timers.length}
            </Text>
          </View>
          <View className="items-center">
        ...
        <View className="mb-6">
          <View className="flex-row justify-between items-center mb-3">
            ...
            <TouchableOpacity
              className={`px-4 py-1.5 rounded-full ${isEditing ? "bg-[#2ECC71]" : "bg-slate-800"}`}
              onPress={() => {
                if (isEditing) {
                  Object.entries(editedLogs).forEach(([taskId, completed]) => {
          ...
          <View className="bg-white rounded-3xl p-5 shadow-sm border border-slate-50">
                ...
                <View key={task.id}>
                  ...
                  >
                    <View className="flex-row items-center">
                      ...
                      <Text
                        className={`ml-3 text-sm ${isCompleted ? "text-slate-700" : "text-slate-400"}`}
                      >
                        {task.name}
                      </Text>
                    </View>
            ...
            {habits.length === 0 && (
              <Text className="text-slate-400 text-xs">No habits tracked.</Text>
            )}
          </View>
        </View>
        ...
        <View className="mb-6">
          ...
          <View className="bg-white rounded-3xl p-5 shadow-sm border border-slate-50">
            ...
            {timers.length === 0 && (
              <Text className="text-slate-400 text-xs">No timers tracked.</Text>
            )}
          </View>
        </View>
      </ScrollView>
